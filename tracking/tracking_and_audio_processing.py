...
try:
    ...
    print( 'Robot audio duration:', (len(robot_audio)/fs, "sampling rate: ", fs))
    print(robot_audio.shape)
    video = VideoFileClip(video_path)
    audio = video.audio
    audio.write_audiofile(file_name + "_audio.wav", fps=fs, codec='pcm_s16le')  # You can also use "mp3"

    # Load audio from bytes using soundfile
    camera_audio, fs = sf.read(file_name + "_audio.wav")
    camera_audio = camera_audio[:, 0]
    print(camera_audio.shape)
    print( 'Camera audio duration:', (len(camera_audio)/fs, "sampling rate: ", fs))

    xcorr = np.roll(signal.correlate(camera_audio, robot_audio, mode='same'), -len(robot_audio) // 2)
    index = np.argmax(np.abs(xcorr))
    start_frame = int(index / fs * gopro_fps)
