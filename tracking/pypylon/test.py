        ...
        while camera.IsGrabbing():
            with camera.RetrieveResult(20000, pylon.TimeoutHandling_ThrowException) as result:
                ...
                if platform.system() == 'Linux':
                    ...
                    filename = f"{folder_path}/{datetime.datetime.now(datetime.timezone.utc)}_{image_quality}.jpeg"
                    img.Save(pylon.ImageFileFormat_Jpeg, filename, ipo)
                else:
                    filename = f"{folder_path}/{datetime.datetime.now(datetime.timezone.utc)}_{image_quality}.png"
